...
const ProfilePage = () => {
  ...
  // If user loads after mount (e.g. page refresh), sync once
  useEffect(() => {
    if (user) {
      setSelected({
        subjects:     user.subjects     || [],
  ...
  const toggle = (key, value) => {
    setSelected((prev) => ({
      ...prev,
      [key]: prev[key].includes(value)
        ? prev[key].filter((v) => v !== value)   // deselect
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.patch("/users/study-profile", selected);
      // update zustand so dashboard reflects changes immediately
  ...
  const isNewProfile = !user?.subjects?.length && !user?.interests?.length;
      ...
      <div className="max-w-2xl mx-auto bg-gray-900 rounded-2xl p-8 shadow-xl">
        ...
        <div className="mb-8 flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-white">
              {isNewProfile ? "Setup your study profile" : "Edit your study profile"}
            </h1>
            <p className="text-gray-400 text-sm mt-1">
              {isNewProfile
                ? "This helps us match you with the right study group"
                : "Tap to deselect current choices or pick new ones"}
            </p>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            ...
            className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-semibold py-3 rounded-lg transition-colors"
          >
            {loading ? "Saving..." : isNewProfile ? "Save & Find My Group" : "Save Changes"}
          </button>
        </form>
      </div>
    </div>
